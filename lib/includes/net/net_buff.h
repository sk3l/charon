...
class basic_net_streambuffer : public std::basic_streambuf<charT, traits>
{
public:
   using buff_t = basic_log_streambuffer<charT,traits>;
   using char_ptr_t = charT*;
...
protected:
   ...
   basic_log_streambuffer(size_t size = BUFFER_INIT_SIZE)
      : buffer_(new charT[size]),
        size_(size),
   {
      this->resetPointers(0, size_);
   }

   basic_log_streambuffer(char_ptr_t buffer, size_t size)
